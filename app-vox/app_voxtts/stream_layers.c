...
static void push_playbackground_finished_event(struct ast_channel *chan, int layer_i)
{
	...
	struct ast_json *blob = ast_json_pack("{s: s, s: s}", "eventname", "VoxPlayBackgroundFinished", "eventbody", data);
	if (!blob)
		return;
...
static void push_playbackground_error_event(struct ast_channel *chan, int layer_i, const char *error_message)
{
	...
	struct ast_json *blob = ast_json_pack("{s: s, s: s}", "eventname", "VoxPlayBackgroundError", "eventbody", data);
	if (!blob)
		return;
...
static void push_playbackground_x_request_id_event(struct ast_channel *chan, int layer_i, const char *x_request_id)
{
	...
	struct ast_json *blob = ast_json_pack("{s: s, s: s}", "eventname", "VoxPlayBackgroundXRequestId", "eventbody", data);
	if (!blob)
		return;
...
static void push_playbackground_duration_event(struct ast_channel *chan, int layer_i, double duration_secs)
{
	...
	struct ast_json *blob = ast_json_pack("{s: s, s: s}", "eventname", "VoxPlayBackgroundDuration", "eventbody", data);
	if (!blob)
		return;
...
static void push_playbackground_streaming_started_event(struct ast_channel *chan, int layer_i)
{
	char data[64];
	snprintf(data, sizeof(data), "%d", layer_i);
	struct ast_json *blob = ast_json_pack("{s: s, s: s}", "eventname", "VoxPlayBackgroundStreamingStarted", "eventbody", data);
	if (!blob)
		return;
...
static void push_playbackground_custom_event(struct ast_channel *chan, int layer_i, const char *event)
{
	...
	struct ast_json *blob = ast_json_pack("{s: s, s: s}", "eventname", "VoxPlayBackgroundEvent", "eventbody", data);
	ast_free(data);
	if (!blob)
		return;
	...
	*arg_sep = '\0';
	...
	while (*arg) {
		...
		if (sep)
			*sep = '\0';
		char *str_value;
		if ((str_value = check_param(arg, "speed"))) {
		...
		} else if ((str_value = check_param(arg, "tone"))) {
			double value = strtod(str_value, &eptr);
			if (*str_value && !*eptr) {
				conf->tone = value;
			} else {
		...
		} else if ((str_value = check_param(arg, "model"))) {
			ast_free(conf->model);
			conf->model = ast_strdup(unescape_str_inplace(str_value));
